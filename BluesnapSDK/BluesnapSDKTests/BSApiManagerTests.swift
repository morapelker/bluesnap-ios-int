...
class BSApiManagerTests: XCTestCase {
    ...
    private func updateShopperWithToken(shopper: BSShopper) {
        ...
        createTokenWithShopperId(shopperId: shopper.vaultedShopperId, completion: { token, error in
            ...
            let cvv = "111"
            let exp = "10/2020"
            let shippingDetails = self.getShippingDetails()
            ...
            if (BSPaymentType.CreditCard.rawValue == shopper.chosenPaymentMethod?.chosenPaymentMethodType) {
                ...
                BSApiManager.submitPurchaseDetails(ccNumber: ccn, expDate: exp, cvv: cvv, last4Digits: nil, cardType: nil, billingDetails: nil, shippingDetails: shippingDetails, fraudSessionId: nil, completion: {
                    (result, error) in
    ...
    private func getShippingDetails() -> BSShippingAddressDetails {
        let shippingDetails = BSShippingAddressDetails()
        shippingDetails.name = "John Doe2"
        shippingDetails.country = "US"
        shippingDetails.state = "NJ"
        shippingDetails.address = "shipping address2"
        shippingDetails.city = "shipping city2"
        shippingDetails.zip = "123456"
    ...
    private func createShopperForUpdate(chosenPaymentMethodType: String, cc: BSCreditCard? = nil) -> BSShopper {
        ...
        shopper.address2 = "some address2"
